    ...
    const contentGenerator = {
        ...
        generateFullHtml(allChatsData) {
            ...
            const embeddedScript = `const allChatsData = ${JSON.stringify(allChatsData)}; const chatView = document.getElementById('main-chat-view'); const sidePanelLinks = document.querySelectorAll('.chat-list-item'); const panelToggleButton = document.getElementById('panel-toggle-btn');
                ...
                function saveChanges() { const originalTitle = document.title.replace(' - 채팅 로그', ''); const fileName = \`\${originalTitle}_수정본.html\`; document.querySelector('.save-changes-container').style.display = 'none'; downloadFile(document.documentElement.outerHTML, fileName, 'text/html;charset=utf-8'); }`;
            ...
            return `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>전체 채팅 백업</title><style>${fullHtmlStyle}</style><script src="https://cdn.jsdelivr.net/npm/marked@4.3.0/marked.min.js"><\/script></head><body>
                ...
                <div class="floating-buttons"><button id="edit-mode-btn" class="floating-btn" title="편집 모드" onclick="toggleEditMode()">${ICONS.edit}</button><button class="floating-btn" title="맨 위로" onclick="window.scrollTo({ top: 0, behavior: 'smooth' })">${ICONS.arrowUp}</button><button class="floating-btn" title="맨 아래로" onclick="window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' })">${ICONS.arrowDown}</button></div>
