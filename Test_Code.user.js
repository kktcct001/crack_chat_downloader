...
(function() {
    ...
    const app = {
        ...
        findMobileInjectTarget() {
            ...
            const sideMenuPanel = anchorElement.parentElement;
            if (!sideMenuPanel) return null;
            
            // 패널 내에서 'eh9908w0'를 포함하는 모든 div 요소를 찾습니다.
            const candidates = Array.from(sideMenuPanel.querySelectorAll('div[class*="eh9908w0"]'));
            
            // 후보들 중에서 기준점(메뉴 목록)이 아닌 마지막 요소를 반환합니다.
            // 이것이 최하단 버튼 컨테이너일 확률이 가장 높습니다.
            const target = candidates.filter(el => !el.isEqualNode(anchorElement)).pop();
