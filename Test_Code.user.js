...
(function() {
    ...
    const apiHandler = {
        ...
        async fetchAllChatrooms(accessToken, onPageLoad) {
            let allRooms = []; let nextCursor = null; let pageCount = 1; let hasMore = true;
    ...
    const contentGenerator = {
        generateTxt(messages) {
            return messages.map(msg => `[${msg.role === 'user' ? '사용자' : '캐릭터'} message]\n${msg.content}`).join('\n\n===\n\n');
        },
        ...
        generateFullHtml(allChatsData) {
            const sidePanelHtml = allChatsData.map((chatData, index) => {
                ...
                return `<a href="#" class="chat-list-item" onclick="renderChat(${index}); return false;">
            ...
            @media (max-width: 768px) { #panel-toggle-btn { display: none; } #chat-list-panel { box-shadow: 2px 0 5px rgba(0,0,0,0.1); } body{padding-bottom:80px}body.edit-mode{padding-bottom:60px}body.edit-mode .floating-buttons{opacity:0;visibility:hidden;pointer-events:none;transition:none}.floating-buttons{transition:opacity .3s,bottom .3s,visibility .3s,pointer-events 0s .3s;}.floating-buttons.init-hide{opacity:0;visibility:hidden;pointer-events:none}.floating-buttons.visible{opacity:1;visibility:visible;pointer-events:auto}.floating-btn{width:50px;height:50px;font-size:28px}.floating-btn svg{width:24px;height:24px}}
    ...
    const utils = {
        downloadFile(content, fileName, mimeType) {
            const a = document.createElement('a'); const blob = new Blob([content], { type: mimeType }); a.href = URL.createObjectURL(blob); a.download = fileName; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(a.href);
        },
        async copyToClipboard(text, statusEl, successMsg) {
            try { await navigator.clipboard.writeText(text); this.updateStatus(statusEl, `${successMsg} 클립보드에 복사되었습니다.`); } catch (err) { this.updateStatus(statusEl, '클립보드 복사에 실패했습니다.', true); console.error('Clipboard copy failed:', err); }
        },
        updateStatus(statusEl, message, isError = false) {
            if(!statusEl) return; statusEl.textContent = message; statusEl.style.color = isError ? '#FF4432' : '#85837D';
        }
    };
