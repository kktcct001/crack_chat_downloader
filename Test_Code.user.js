...
// 영감을 받아, 기존 DOM 스크래핑 방식에서 API 호출 방식으로 로직을 재구성하였습니다.
...
(function() {
    ...
    const SELECTORS = {
        ...
        panel: { overlay: '.downloader-panel-overlay', closeBtn: '#downloader-close-btn', statusText: '.status-text' },
    };
    ...
    const ICONS = {
        ...
        panelToggle: `<svg width="20" height="20" viewBox="0 0 20 20" fill="none"><g style="transform-origin: center center;"><path id="toggle_bar" d="M4.16667 2.5H2.5V17.5H4.16667V2.5Z" fill="#85837D" style="transition: transform 0.3s ease-in-out;"></path><path id="toggle_open_arrow" d="M8.54643 12.9461L9.72494 14.1246L12.6712 11.1783L12.9658 10.8837C13.454 10.3955 13.454 9.60406 12.9658 9.11591L12.6712 8.82128L9.72494 5.875L8.54643 7.05351L10.6594 9.16646H2.5V10.8331H10.6594L8.54643 12.9461Z" fill="#85837D" style="transition: opacity 0.3s ease-in-out; opacity: 0;"></path><path id="toggle_close_arrow" d="M11.4535 7.05351L10.275 5.875L7.32871 8.82128L7.03409 9.11591C6.54593 9.60406 6.54593 10.3955 7.03409 10.8837L7.32871 11.1783L10.275 14.1246L11.4535 12.9461L9.34056 10.8331L17.4999 10.8331V9.16645L9.34056 9.16646L11.4535 7.05351Z" fill="#85837D" style="transition: opacity 0.3s ease-in-out; opacity: 0;"></path></g></svg>`,
    ...
    const apiHandler = {
        ...
        async fetchAllMessages(chatroomId, accessToken) {
            // [턴 수 상한 수정 가이드 1/3]
            // 아래의 'limit=2000'은 한 번에 불러올 메시지의 최대 개수임 (2000개 = 1000턴)
            // 만약 턴 수 상한을 2000턴으로 올리고 싶다면, 이 값을 'limit=4000'으로 변경할 것
            // [!주의!] 이 값을 너무 높이면 서버에서 요청을 거부할 수 있음. 4000개(2000턴) 정도 권장
    ...
    const contentGenerator = {
        generateTxt(messages) { return messages.map(msg => `[${msg.role === 'user' ? 'user' : 'assistant'} message]\n\n${msg.content}`).join('\n\n===\n\n'); },
        ...
        generateFullHtml(allChatsData, currentChatroomId) {
            ...
            #chat-list-panel{position:fixed;top:0;left:0;width:260px;height:100%;background-color:#fff;border-right:1px solid #e9ecef;transform:translateX(-100%);transition:transform .3s ease-in-out;z-index:1001;display:flex;flex-direction:column}#chat-list-panel.is-open{transform:translateX(0)}.panel-header{display:flex;align-items:center;justify-content:space-between;padding:16px;font-size:16px;font-weight:600;color:var(--text_secondary);border-bottom:1px solid #e9ecef;flex-shrink:0}.panel-header .title{font-size:16px;font-weight:700;color:var(--text_primary);}.user-note-btn{padding:4px 8px;font-size:12px;font-weight:600;background-color:#f1f3f5;border:1px solid #dee2e6;color:var(--text_secondary);border-radius:4px;cursor:pointer}.user-note-btn:disabled{background-color:#f8f9fa;color:#adb5bd;cursor:not-allowed}.panel-scroll-area{overflow-y:auto;flex-grow:1}.chat-list-item{display:flex;align-items:center;padding:12px 16px;text-decoration:none;color:inherit;border-bottom:1px solid #f1f3f5;cursor:pointer}.chat-list-item:hover{background-color:#f8f9fa}.chat-list-item.active{background-color:#FFF3F2}.list-item-avatar{width:36px;height:36px;border-radius:50%;overflow:hidden;margin-right:12px;flex-shrink:0}.list-item-avatar img{width:100%;height:100%;object-fit:cover}.list-item-content{overflow:hidden}.list-item-name{font-weight:600;font-size:14px;color:var(--text_primary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin:0 0 4px}.list-item-topic{font-size:13px;color:var(--text_secondary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin:0}
            #panel-toggle-btn{position:fixed;top:50%;left:0;transform:translateY(-50%);width:24px;height:48px;background-color:#fff;border:1px solid #e9ecef;border-left:none;border-radius:0 4px 4px 0;cursor:pointer;z-index:1000;display:flex;align-items:center;justify-content:center;padding:0;transition:all .2s ease-in-out}body.panel-open-pc #panel-toggle-btn{left:260px;z-index:1002;}#panel-toggle-btn svg g{display:flex;align-items:center;justify-content:center}#panel-toggle-btn:hover #toggle_bar{transform:translateX(4px)}#panel-toggle-btn:hover #toggle_open_arrow{opacity:1}body.panel-open-pc #panel-toggle-btn:hover #toggle_bar{transform:translateX(-4px)}body.panel-open-pc #panel-toggle-btn:hover #toggle_open_arrow{opacity:0}body.panel-open-pc #panel-toggle-btn:hover #toggle_close_arrow{opacity:1}
            ...
            @media (max-width: 768px) { #panel-toggle-btn { display: none; } #chat-list-panel { box-shadow: 2px 0 5px rgba(0,0,0,0.1); } body{padding-bottom:80px} body.panel-open-pc{padding-left:0;} #main-content-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.5);z-index:1000;display:none;opacity:0;transition:opacity .3s ease-in-out}body.panel-open-mob #main-content-overlay{display:block;opacity:1} body.edit-mode{padding-bottom:60px}body.edit-mode .floating-buttons, body.panel-open-mob .floating-buttons{opacity:0;visibility:hidden;pointer-events:none;transition:none}.floating-buttons{transition:opacity .3s,bottom .3s,visibility .3s,pointer-events 0s .3s;}.floating-buttons.init-hide{opacity:0;visibility:hidden;pointer-events:none}.floating-buttons.visible{opacity:1;visibility:visible;pointer-events:auto}.floating-btn{width:50px;height:50px;font-size:28px}.floating-btn svg{width:24px;height:24px}}
            ...
            const embeddedScript = `const allChatsData = ${JSON.stringify(allChatsData)}; const currentChatroomId = "${currentChatroomId}"; const chatView = document.getElementById('main-chat-view'); const sidePanelLinks = document.querySelectorAll('.chat-list-item'); const panelToggleButton = document.getElementById('panel-toggle-btn');
                function renderChat(index) { const chatData = allChatsData[index]; if (!chatData) return; const character = chatData.character; const characterName = character?.name || chatData.title || '알 수 없는 채팅'; const messagesHtml = chatData.messages.map(msg => { const roleClass = msg.role === 'user' ? 'user' : 'assistant'; const contentHtml = marked.parse(msg.content || '').replace(/<p>/g, '<div>').replace(/<\\/p>/g, '</div>'); const actionButtonHtml = \`<div class="message-actions"><button class="action-btn delete-btn" title="메시지 삭제">${ICONS.trash}</button><div class="message-checkbox" title="메시지 선택"><span class="checkbox-icon unchecked">${ICONS.unchecked}</span><span class="checkbox-icon checked">${ICONS.checked}</span></div></div>\`; return \`<div class="message-wrapper \${roleClass}">\${roleClass === 'assistant' ? \`<div class="character-name-wrapper"><div class="character-name">\${characterName}</div></div>\` : ''}<div class="message-bubble \${roleClass}-bubble">\${contentHtml}\${actionButtonHtml}</div></div>\`; }).join(''); chatView.innerHTML = \`<div class="chat-container">\${messagesHtml}</div>\`; sidePanelLinks.forEach(link => link.classList.remove('active')); sidePanelLinks[index].classList.add('active'); document.title = characterName + ' - 채팅 로그'; window.scrollTo({ top: 0, behavior: 'auto' }); if (document.getElementById('chat-list-panel').classList.contains('is-open') && window.innerWidth <= 768) { toggleChatList(); } manageUserNoteButton(chatData); }
                function manageUserNoteButton(chatData) { const btn = document.getElementById('user-note-btn'); if(!btn) return; const userNoteContent = chatData.userNote ? chatData.userNote.content : null; if(userNoteContent && userNoteContent.trim() !== '') { btn.disabled = false; btn.onclick = () => showUserNoteModal(userNoteContent); } else { btn.disabled = true; btn.onclick = null; } }
                function showUserNoteModal(content) { const modal = document.createElement('div'); modal.className = 'un-modal-overlay'; modal.innerHTML = \`<div class="un-modal-content"><div class="un-modal-header"><span>유저 노트</span><button class="un-modal-close-btn">${ICONS.close}</button></div><div class="un-modal-body">\${content || '입력된 유저노트가 없습니다.'}</div></div>\`; const close = () => modal.remove(); modal.querySelector('.un-modal-close-btn').onclick = close; modal.onclick = (e) => { if(e.target === modal) close(); }; document.body.appendChild(modal); }
                function toggleChatList() { const isMobile = window.innerWidth <= 768; document.getElementById('chat-list-panel').classList.toggle('is-open'); document.body.classList.toggle(isMobile ? 'panel-open-mob' : 'panel-open-pc'); }
                document.addEventListener('DOMContentLoaded', () => { marked.setOptions({ gfm: true, breaks: true }); let initialIndex = 0; if (currentChatroomId) { const foundIndex = allChatsData.findIndex(chat => chat._id === currentChatroomId); if(foundIndex > -1) initialIndex = foundIndex; } if (allChatsData.length > 0) renderChat(initialIndex); chatView.addEventListener('click', handleContainerClick); document.querySelectorAll('.chat-list-item').forEach(item => item.addEventListener('click', (e) => { e.preventDefault(); renderChat(parseInt(e.currentTarget.dataset.index)); })); });
                ...
                function saveChanges() { const originalTitle = document.title.split(' - ')[0]; const fileName = \`\${originalTitle}_채팅로그_수정본.html\`; document.querySelector('.save-changes-container').style.display = 'none'; downloadFile(document.documentElement.outerHTML, fileName, 'text/html;charset=utf-8'); }
                if (window.matchMedia("(max-width: 768px)").matches) { const floatingButtons = document.querySelector('.floating-buttons'); if (floatingButtons) { floatingButtons.classList.add('init-hide'); let scrollTimeout; window.addEventListener('scroll', () => { if (document.body.classList.contains('edit-mode') || document.body.classList.contains('panel-open-mob')) return; clearTimeout(scrollTimeout); floatingButtons.classList.add('visible'); scrollTimeout = setTimeout(() => { floatingButtons.classList.remove('visible'); }, 1500); }); } }`;
            ...
            return `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>전체 채팅 백업</title><style>${fullHtmlStyle}</style><script src="https://cdn.jsdelivr.net/npm/marked@4.3.0/marked.min.js"><\/script></head><body>
                <button id="panel-toggle-btn" title="대화 목록" onclick="toggleChatList()">${ICONS.panelToggle}</button>
                <div id="main-content-overlay" onclick="toggleChatList()"></div>
                <div id="chat-list-panel"><div class="panel-header"><span class="title">대화 내역</span><button id="user-note-btn" disabled>유저 노트</button></div><div class="panel-scroll-area">${sidePanelHtml}</div></div>
                <div id="main-chat-view"></div>
                <div class="floating-buttons">
                    <button id="mobile-list-btn" class="floating-btn" title="대화 목록" onclick="toggleChatList()">${ICONS.list}</button>
                ...
                <div id="edit-action-bar"><span id="selection-count">0개 메시지 선택됨</span><div class="action-bar-buttons"><button id="bulk-delete-btn" class="action-bar-btn" title="선택한 메시지 삭제" onclick="handleBulkDelete()">${ICONS.trash}</button><button id="exit-edit-mode-btn" class="action-bar-btn" title="편집 종료" onclick="toggleEditMode()">${ICONS.close}</button></div></div>
    ...
    const app = {
        ...
        injectStyles() {
            GM_addStyle(`
                .chat-log-downloader-btn-desktop { display:flex; align-items:center; justify-content:center; height:34px; padding:0 12px; margin:0 8px; border-radius:8px; cursor:pointer; font-size:14px; font-weight:600; color:#FF4432; background-color:#fff; border:1px solid #FF4432; white-space:nowrap; gap:6px; transition: background-color .2s, color .2s; }
                .chat-log-downloader-btn-desktop:hover { background-color:#FFF3F2; }
                ...
                .tab-content-wrapper { padding-top: 24px; min-height: 330px; }
                ...
                .option-group { background-color:#F8F9FA; border-radius:8px; padding:16px; margin-bottom:24px; display:flex; flex-direction:column; gap:16px; }

                #tab-content-current { justify-content: space-between; }
                ...
                #tab-content-current .save-order-btn { flex:1; padding:12px; border-radius:8px; border:1px solid #DEE2E6; font-size:15px; font-weight:600; cursor:pointer; transition:all .2s; background-color:#FFF; color:#495057; }
                #tab-content-current .save-order-btn.active { background-color:#FF4432; color:#fff; border-color:#FF4432; }
                #tab-content-current .format-buttons { display:grid; grid-template-columns: 1fr 1fr 1fr; gap:10px; }
                #tab-content-current .format-btn { display:flex; align-items:center; justify-content:center; gap: 8px; padding:16px; border-radius:8px; border:1px solid #FF4432; font-size:16px; font-weight:700; cursor:pointer; background-color:#FF4432; color:#fff; transition: background-color .2s; }
                #tab-content-current .format-btn:hover { background-color: #E03E2D; }
                #tab-content-current .format-btn:disabled { background-color:#ADB5BD; border-color:#ADB5BD; cursor:not-allowed; }
                .checkbox-group { display:flex; align-items:center; justify-content:center; gap:8px; margin-top:24px; }
                .checkbox-group label { margin:0; font-size:14px; color:#333; cursor:pointer; }
                .checkbox-group input { cursor:pointer; }

                #tab-content-full { justify-content: flex-end; }
                .warning-box { background-color: #FFFBEB; border: 1px solid #FDE68A; border-radius: 8px; padding: 16px; margin-bottom: 24px; }
                ...
                .full-save-btn { display:flex; align-items:center; justify-content:center; gap:8px; width: 100%; padding: 16px; border-radius: 8px; border: none; font-size: 16px; font-weight: 700; cursor:pointer; background-color:#FF4432; color:#fff; transition: background-color 0.2s; }
                .full-save-btn:hover { background-color: #E03E2D; }
                .full-save-btn:disabled { background-color:#ADB5BD; cursor:not-allowed; }
                ...
                <div class="tab-content-wrapper">
                    <div id="tab-content-current" class="tab-content active">
                        <div class="option-group">
                            ...
                            <div class="input-group"><label>저장 순서</label><div class="save-order-buttons"><button class="save-order-btn ${isOldestActive}" data-order="oldest">시작 대화부터</button><button class="save-order-btn ${isLatestActive}" data-order="latest">최신 대화부터</button></div></div>
                        </div>
                    ...
                    <div id="tab-content-full" class="tab-content">
                        <div class="warning-box"><div class="warning-header"><span>⚠</span><span>서버 부하에 주의하세요</span></div><div class="warning-content"><p>전체 채팅 저장 기능은 서버에게서 전체 채팅방 목록을 받아오고, 그다음 각 채팅방의 대화 내용을 하나씩 순서대로 요청하여 가져옵니다.</p><p>이 기능은 짧은 시간 동안 서버에 많은 요청을 보냅니다. 무분별한 사용은 서버에 부하를 초래할 수 있습니다. 꼭 필요할 때만 신중히 사용해 주세요.</p></div></div>
                        <button class="full-save-btn"><span>HTML로 전체 저장</span></button>
                    </div>
                </div>
        ...
        async startCurrentDownloadProcess(button) {
            ...
            document.querySelectorAll('.format-btn').forEach(btn => btn.disabled = true);
            ...
            try {
                ...
                // 만약 턴 수 상한을 2000턴으로 올리고 싶다면, 이 값을 'turnCount > 2000'으로 변경
                // [!권장!] '턴 수는 1에서 1000 사이여야 합니다.' 오류 메시지 숫자는 고치면 좋고 안 고쳐도 상관없음
                ...
                let allMessages = await apiHandler.fetchAllMessages(chatInfo.chatroomId, accessToken); if (!allMessages.length) throw new Error('불러올 대화 기록이 없습니다.');
                ...
                else { fileContent = contentGenerator.generateJson(messagesToProcess); clipboardContent = fileContent; }
        ...
        async startFullDownloadProcess(button) {
            ...
            try {
                ...
                for (let i = 0; i < chatrooms.length; i++) {
                    ...
                    try { const messages = await apiHandler.fetchAllMessages(room._id, accessToken); allChatsData.push({ ...room, messages }); }
                ...
                utils.updateStatus(statusEl, '전체 대화 내용 HTML 파일을 생성하는 중...', 'info');
                const currentChatInfo = apiHandler.getChatInfo();
                const fullHtmlContent = contentGenerator.generateFullHtml(allChatsData, currentChatInfo ? currentChatInfo.chatroomId : null);
                const timestamp = new Date().toISOString().slice(0, 10);
                utils.downloadFile(fullHtmlContent, `Crack_full_chat_${timestamp}.html`, 'text/html;charset=utf-8');
