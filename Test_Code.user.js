...
(function() {
    ...
    // ... (CONFIG, SELECTORS, ICONS, apiHandler 객체는 변경 없음) ...
    const CONFIG = {
    ...
    const contentGenerator = {
        ...
        generateHtml(messages, characterName) {
            ...
            const embeddedScript = `
                // 즉시 실행 함수 (IIFE)로 스크립트 전체를 감싸서 전역 스코프 오염 방지
                (function() {
                    const ICONS = {
                        close: \`${ICONS.close}\`,
                        edit: \`${ICONS.edit}\`
    ...
    const app = {
        ...
        findMobileInjectTarget() {
            const innerMenuList = document.querySelector('.css-uxwch2');
            if (innerMenuList) {
                const sideMenuPanel = innerMenuList.closest('div[class*="eh9908w0"]');
