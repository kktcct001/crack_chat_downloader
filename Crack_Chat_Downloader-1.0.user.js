...
(function() {
    ...
    GM_addStyle(`
        ...
        @media (max-width: 768px) { .downloader-panel { width: 90%; } }
    ...
    class ApiHandler {
        ...
        async fetchAllMessages(chatroomId, maxTurnsToSave, updateStatus) {
            ...
            updateStatus(`메시지 ${messagesToFetch}개를 불러오는 중...`);
    ...
    function generateHtmlContent(messages, characterName) {
        ...
        const messageHtml = reversedMessages.map(msg => {
...
<body>
    ...
    async function startDownloadProcess(format) {
        ...
        try {
            ...
            const messages = await apiHandler.fetchAllMessages(chatInfo.chatroomId, turnCount, (status) => { statusText.textContent = status; });
            if (!messages.length) throw new Error('불러올 대화 기록이 없습니다.');
            ...
            if (format === 'html') {
                downloadFile(generateHtmlContent(messages, characterName), fileName, 'text/html;charset=utf-8');
            } else if (format === 'txt') {
                downloadFile(generateTxtContent(orderedMessages), fileName, 'text/plain;charset=utf-8');
            } else if (format === 'json') {
                downloadFile(generateJsonContent(messages), fileName, 'application/json;charset=utf-8');
            }

            const downloadSuccessMessage = `다운로드 성공! 총 ${orderedMessages.length}개 메시지 저장.`;

            if (shouldCopyToClipboard) {
                await copyToClipboard(generateTxtContent(orderedMessages), statusText, downloadSuccessMessage);
            } else {
