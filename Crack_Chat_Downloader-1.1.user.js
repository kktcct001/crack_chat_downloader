...
(function() {
    ...
    const contentGenerator = {
        ...
        generateHtml(messages, characterName) {
            ...
            renderer.heading = (text, level) => `<h${level} class="css-fkmgr6">${text}</h${level}>`;
            renderer.strong = (text) => `<strong class="css-sd7959">${text}</strong>`;
            renderer.em = (text) => `<em class="css-1qbs0mm">${text}</em>`;
            renderer.list = (body, ordered) => `<${ordered ? 'ol' : 'ul'} class="${ordered ? 'css-yggfb7' : 'css-1ostlo9'}">${body}</${ordered ? 'ol' : 'ul'}>`;
            ...
            renderer.code = (code) => `<pre class="code-block"><code>${code.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</code></pre>`;
            delete renderer.paragraph;
            marked.setOptions({ renderer, gfm: true, breaks: true });

            const messageHtml = messages.map(msg => {
                let contentHtml = marked.parse(msg.content || '');
                contentHtml = contentHtml.replace(/<p>/g, `<div class="${msg.role === 'user' ? 'css-192kozn' : 'css-1dfojlr'}">`).replace(/<\/p>/g, '</div>');
    ...
    const app = {
        ...
        showPopupPanel() {
            ...
            panelOverlay.innerHTML = `<div class="downloader-panel"><div class="downloader-header"><h2 class="downloader-title">대화 저장 설정</h2><button id="downloader-close-btn" class="downloader-close-btn">${ICONS.close}</button></div><div class="downloader-content"><div><div class="input-group"><label for="message-count-input">저장할 턴 수 (최대 1000)</label><input type="number" id="message-count-input" value="30" min="1" max="1000"></div><div class="format-buttons"><button data-format="html" class="format-btn">HTML</button><button data-format="txt" class="format-btn">TXT</button><button data-format="json" class="format-btn">JSON</button></div><div class="checkbox-group"><input type="checkbox" id="copy-clipboard-checkbox"><label for="copy-clipboard-checkbox">클립보드에 복사하기</label></div></div><p id="downloader-status-text" class="status-text"></p></div></div>`;
        ...
        async startDownloadProcess(format) {
            ...
            try {
                ...
                const chronological = [...allMessages].reverse().slice(0, turnCount * 2);
                ...
                switch (format) {
                    case 'html':
                        fileContent = contentGenerator.generateHtml(chronological, characterName);
                        clipboardContent = contentGenerator.generateTxt(reverseChronological);
                        break;
                    case 'txt':
                        fileContent = contentGenerator.generateTxt(reverseChronological);
                        clipboardContent = fileContent;
                        break;
                    case 'json':
                        fileContent = contentGenerator.generateJson(reverseChronological);
                        clipboardContent = fileContent;
                        break;
                }
                ...
                const successMsg = `다운로드 성공! 총 ${chronological.length / 2}턴(${chronological.length}개) 저장.`;
